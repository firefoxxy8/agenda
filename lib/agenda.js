...
/**
 * If the app has crashed while a job was running, use this to unlock zombies upon restart.
 * @todo If agenda is running in a distributed system, it should additionally take into
 * consideration the mac address of the machine in addition to the process id.
 */
Agenda.prototype.unlockZombies = function(cb) {
  var self = this;
  ps.lookup({ psargs: 'ux' }, function(err, processes) {
    if (err) {
      throw new Error( err );
    }
    var processIds = processes.map(function(process){ return process.id; });
    self._db.update({lockedByProcess: { $exists: true, $ne: null, $nin: processIds }, lockedAt: { $exists: true, $ne: null } }, { $set: { lockedAt: null } }, {multi: true}, cb);
  });
};
...
Agenda.prototype._findAndLockNextJob = function(jobName, definition, cb) {
  ...
  this._db.findAndModify(
    ...
    {'priority': -1},
    {$set: { lockedAt: now, lockedByProcess: process.pid }},
    {'new': true},
...
function processJobs(extraJob) {
  ...
  if(!extraJob) {
    ...
    var now = new Date();
    self._db.findAndModify({ _id: extraJob.attrs._id, lockedAt: null }, {}, { $set: { lockedAt: now, lockedByProcess: process.pid } }, function(err, resp) {
      if(resp) {
